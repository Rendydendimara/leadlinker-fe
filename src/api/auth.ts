...
export const ApiSendOTPResetPassword = async (email: string) => {
  const response = await axios
    .post(`${BACKEND_URL}/auth/send-otp-forgot-password`, { email })
    .then((response) => {
      return response;
    })
    .catch((error) => {
...
export const ApiVerifyForgotPassword = async (data: {
  userId: string;
  password: string;
  otpResetPassword: string;
}) => {
  const response = await axios
    .post(`${BACKEND_URL}/auth/verify-forgot-password`, data)
    .then((response) => {
      return response;
    })
    .catch((error) => {
